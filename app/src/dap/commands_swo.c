...
int32_t dap_handle_command_swo_transport(const struct device *dev) {
    ...
    uint8_t transport = 0;
    CHECK_EQ(ring_buf_get(&data->buf.request, &transport, 1), 1, -EMSGSIZE);
...
int32_t dap_handle_command_swo_mode(const struct device *dev) {
    ...
    uint8_t mode = 0;
    CHECK_EQ(ring_buf_get(&data->buf.request, &mode, 1), 1, -EMSGSIZE);
...
int32_t dap_handle_command_swo_baudrate(const struct device *dev) {
    ...
    uint32_t baudrate = 0;
    CHECK_EQ(ring_buf_get(&data->buf.request, (uint8_t*) &baudrate, 4), 4, -EMSGSIZE);
    CHECK_EQ(ring_buf_put(&data->buf.response, &((uint8_t) {DAP_COMMAND_SWO_BAUDRATE}), 1), 1, -ENOBUFS);
...
int32_t dap_handle_command_swo_control(const struct device *dev) {
    ...
    uint8_t control = 0;
    CHECK_EQ(ring_buf_get(&data->buf.request, &control, 1), 1, -EMSGSIZE);
...
int32_t dap_handle_command_swo_status(const struct device *dev) {
    ...
    CHECK_EQ(ring_buf_put(&data->buf.response, &((uint8_t) {DAP_COMMAND_SWO_STATUS}), 1), 1, -ENOBUFS);
    ...
    CHECK_EQ(ring_buf_put(&data->buf.response, &trace_status, 1), 1, -ENOBUFS);
...
int32_t dap_handle_command_swo_extended_status(const struct device *dev) {
    ...
    uint8_t control = 0;
    CHECK_EQ(ring_buf_get(&data->buf.request, &control, 1), 1, -EMSGSIZE);
    CHECK_EQ(ring_buf_put(&data->buf.response, &((uint8_t) {DAP_COMMAND_SWO_EXTENDED_STATUS}), 1), 1, -ENOBUFS);
...
int32_t dap_handle_command_swo_data(const struct device *dev) {
    ...
    CHECK_EQ(ring_buf_put(&data->buf.response, &((uint8_t) {DAP_COMMAND_SWO_DATA}), 1), 1, -ENOBUFS);
    ...
    *response_count = 0;
    ...
    do {
        uint8_t *swo_ptr;
        uint32_t swo_read = ring_buf_get_claim(&data->buf.swo, &swo_ptr, count);
        uint8_t *response_ptr;
        uint32_t response_write = ring_buf_put_claim(&data->buf.response, &response_ptr, swo_read);
        memcpy(response_ptr, swo_ptr, response_write);
        int32_t get_ret = ring_buf_get_finish(&data->buf.swo, response_write);
        int32_t put_ret = ring_buf_put_finish(&data->buf.response, response_write);
        if (get_ret != 0 || put_ret != 0) { return -ENOBUFS; }
        *response_count += response_write;
    } while (
        *response_count < count &&
        ring_buf_size_get(&data->buf.swo) > 0 &&
