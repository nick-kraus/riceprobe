...
#define DAP_OUT_EP_INDEX			0
#define DAP_IN_EP_INDEX 			1
...
static void dap_usb_if_config(struct usb_desc_header *head, uint8_t bInterfaceNumber) {
    ARG_UNUSED(head);

    dap_usb_descriptor.iad.bFirstInterface = bInterfaceNumber;
    dap_usb_descriptor.intf.bInterfaceNumber = bInterfaceNumber;
    dap_usb_descriptor.intf.iInterface = usb_get_str_descriptor_idx(&dap_interface_string_descriptor);
...
static void dap_write_complete_cb(uint8_t ep_addr, int size, void *priv) {
    LOG_DBG("wrote %d bytes to USB ep 0x%x", size, ep_addr);
}

static void dap_transfer_cb(uint8_t ep_addr, int size, void *priv) {
    ...
    if (size > 0) {
        usb_transfer(
            cfg->endpoint[DAP_IN_EP_INDEX].ep_addr,
            ep_buf,
            size,
            USB_TRANS_WRITE,
            dap_write_complete_cb,
            cfg
        );
    }
...
static void dap_usb_status_cb(
	...
	case USB_DC_CONFIGURED:
        ...
        dap_transfer_cb(cfg->endpoint[DAP_OUT_EP_INDEX].ep_addr, 0, cfg);
		break;
	case USB_DC_SUSPEND:
		LOG_INF("USB device suspended");
        usb_cancel_transfer(cfg->endpoint[DAP_OUT_EP_INDEX].ep_addr);
		break;
	default:
		break;
	}
...
USBD_DEFINE_CFG_DATA(dap_usb_config) = {
	.usb_device_description = NULL,
	.interface_descriptor = &dap_usb_descriptor.intf,
	.interface_config = dap_usb_if_config,
