...
int32_t dap_handle_command_swd_configure(const struct device *dev) {
    ...
    CHECK_EQ(ring_buf_get(&data->buf.request, &configuration, 1), 1, -EMSGSIZE);
...
int32_t dap_handle_command_swd_sequence(const struct device *dev) {
    ...
    CHECK_EQ(ring_buf_put(&data->buf.response, &((uint8_t) {DAP_COMMAND_SWD_SEQUENCE}), 1), 1, -ENOBUFS);
    ...
    for (uint8_t i = 0; i < seq_count; i++) {
        ...
        if ((info & info_mode_mask) != 0) {
            ...
            while (swclk_cycles > 0) {
                ...
                swdio >>= bits;
                CHECK_EQ(ring_buf_put(&data->buf.response, &swdio, 1), 1, -ENOBUFS);
            }
        } else {
            FATAL_CHECK(
            ...
            while (swclk_cycles > 0) {
                uint8_t swdio = 0;
                CHECK_EQ(ring_buf_get(&data->buf.request, &swdio, 1), 1, -EMSGSIZE);
                uint8_t bits = 8;
                while (bits > 0 && swclk_cycles > 0) {
