...
class TestOpenOCD:
    def test_flash_read(self, openocd_rtt):
    ...
    def test_manipulate_memory(self, openocd_rtt):
    ...
    def test_breakpoint(self, openocd_rtt):
        openocd, rtt = openocd_rtt
        # start in a running state
        openocd.send(b'resume')
        assert(b'running' in openocd.send(b'$_CHIPNAME.cpu curstate'))
    ...
    def test_rtt_transfer(self, openocd_rtt):
        openocd, rtt = openocd_rtt
        # start in a running state
        openocd.send(b'resume')
        assert(b'running' in openocd.send(b'$_CHIPNAME.cpu curstate'))
